...
\begin{abstract}
  To type check a \picalc{} process with linear and shared types without relying on type annotations we need to infer the types of the channels we create within the process.
  If we were to limit ourselves to the shared \picalc{}, we could traverse a process bottom-up and always eagerly solve typing constraints into substitutions that are then applied to the typing context.
  However in our setting, typing constraints do not always come with have a most general unifier, and thus cannot always be eagerly solved.
  We follow Padovani \cite{Padovani15} and provide a \emph{co-contextual typing inference} \cite{ErdwegBKKM15} algorithm that traverses processes bottom-up and, in addition to the typing context, collecting typing constraints too.
  We then solve those constraints with a most general solution by using well-known unification algorithms \cite{McBride03}, and defer the rest until more information becomes available.
