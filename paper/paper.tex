...
\begin{itemize}
\item
  For the expression $\sfst~e$ we first recursively infer the metavariable context $\gamma$, the typing context $\Gamma$, the type $\tau$ and the constraints $Cs$ for $e$.
  We then add two fresh metavariables $?t$ and $?s$ of kind $\ktype$ to $\gamma$, add constraints $\eqconstr{\tau}{\tprod~?t~?s}$ and $\sumconstr{?s}{?s}{?s}$ to $cs$, and return $?t$ as the type of $e$.

\item
  For the expression $\scase~e~p~q$ we first recursively infer $p$ and $q$ and $e$: $\func{inferProc}~p = \gamma_p, (\Gamma_p, s) , Cs_p$, $\func{inferProc}~q = \gamma_q, (\Gamma_q, t) , Cs_q$, and $\func{inferExpr}~e = \gamma_e, \Gamma_e, \tau, Cs_e$.
  We add the metavariables in $\gamma_p$ to the ones in $\gamma_q$ and a new metavariable $?r$ of kind $\ktype$, and refer to the result as $\delta$.
  We add the constraints in $Cs_p$  to the ones in $Cs_q$ and create new constraints $\eqconstr{\Gamma_p}{\Gamma_q}$ 
\end{itemize}
