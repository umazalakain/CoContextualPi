...
\begin{abstract}
  To type check a \picalc{} process with linear and shared types without relying on type annotations we need to infer the types of the channels we create within a process.
  If we were to limit ourselves to the shared \picalc{}, we could traverse a process bottom-up and eagerly solve typing constraints into substitutions that are then applied to the typing context.
  However in our setting, typing constraints do not always come with have a most general solution, and thus cannot always be eagerly solved.
