...
\end{mathpar}
We henceforth use $\ttype{\gamma}$ to stand for $\tkind{\gamma}{\ktype}$ and $\tusage{\gamma}$ to stand for $\tkind{\gamma}{\kusage}$.
...
To keep us from returning the original constraints as output we show that every constraint we return is either unsatisfiable or not uniquely constrained and has to be deferred.
altogether.
\begin{flalign*}
&\func{solve}~cs_1 \equiv (\sigma, cs_2) \to \forall c \in cs_2. ~ \type{IsNotUnique}~c ~ \type{\uplus} ~ \type{IsUnsat}~c &&
\end{flalign*}
where
\begin{flalign*}
